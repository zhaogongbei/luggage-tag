...
## TC-SILENT-01: 本地端客户页静默直打

### 场景
验证本地端或本机服务场景下，客户点击【打印】不会进入浏览器打印页，而是由后台本地打印服务创建订单并发送打印机。
...
## TC-BROWSER-PRINT-01: 浏览器打印回退

### 场景
验证普通浏览器访问生产域名时不尝试静默打印，而是创建可追溯订单后进入浏览器打印页。

### 步骤
1. 使用浏览器访问 `https://tag.ycgg.cc.cd/` 或非 localhost 的 Web 地址。
2. 进入定制页，选择颜色并输入英文姓名。
3. 点击【打印】。
4. 观察网络请求和页面跳转。

### 验收
- 网络请求为 `POST /api/orders`，不是 `POST /api/orders/direct-print`。
- 成功后跳转 `/ticket/:id?autoPrint=1&autoReturn=1`。
- 浏览器显示系统打印窗口或打印预览。
- 订单已保存，后台可按编号追溯和重新打印。

---

## TC-BROWSER-PRINT-02: 打印页手动打印

### 场景
验证浏览器打印页只有在显式 `autoPrint=1` 时才自动打开打印窗口。

### 步骤
1. 打开 `/ticket/:id`。
2. 确认页面不自动打开打印窗口。
3. 点击页面上的【打印】按钮。
4. 打开 `/ticket/:id?autoPrint=1`。

### 验收
- `/ticket/:id` 只显示小票和手动打印按钮。
- 点击【打印】后浏览器打开打印窗口。
- `/ticket/:id?autoPrint=1` 自动触发浏览器打印窗口。

---

## TC-DESKTOP-ADMIN-01: 本地端后台入口

### 场景
验证本地端客户打印页在工作人员登录后可以进入后台。
