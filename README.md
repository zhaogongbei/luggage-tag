...
- 客户定制页点击【打印】后调用 `POST /api/orders/direct-print`
- 后端在本地服务中创建订单、生成编号和时间，并直接发送到后台配置的实体打印机
- 客户页不调用 `window.print()`，不打开 `/ticket` 打印页，不显示浏览器打印预览
- 打印成功后订单状态为 `printed`，页面短暂显示编号后自动清空
- 打印失败时订单保留为 `pending`，后台可按订单编号追溯并重新打印
